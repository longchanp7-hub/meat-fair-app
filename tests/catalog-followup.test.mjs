...
test('nested source paragraphs are dishes, not one combined meat-category title',()=>{
 const html=doc('<div class="menu-card"><h4>焼肉</h4><p class="menu-items"><p class="m-niku">牛カルビ</p><p class="m-niku m-yasumi">牛タン<span>※加工肉です。</span></p><p class="m-niku m-yasumi">中落カルビ<span>※ディナー限定</span></p></p></div>');
 const {items}=extractCatalogPage(html,'https://example.com/menu/',{...brand,id:'stamina-taro'});
 assert.deepEqual(items.map(e=>e.title),['牛タン','中落カルビ']);
 assert.deepEqual(items.find(e=>e.title==='中落カルビ').targetCourses,['ディナー限定']);
});
