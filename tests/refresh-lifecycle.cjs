...
module.exports=async function checkRefreshLifecycle({page,inspect,settle}){
  ...
  const courses=page.locator(card+' [data-gallery-role="fair"] [data-group="course"]'),meals=page.locator(card+' [data-gallery-role="fair"] [data-group="related-meal"]');
  ...
  const mediaRoute=route=>failGallery?route.fulfill({status:503,contentType:'text/plain',body:'test-only temporary unavailability'}):route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({...original.media,fairsUpdatedAt:next.updatedAt})});
  ...
  try{
    ...
    assert.equal(await page.locator(card+' [data-gallery-role="fair"] .media-tile').count(),0,'removed fair retained stale supplementary images');
    assert.equal(await page.locator('[data-brand-card]').count(),1,'refresh lost brand filter');
    assert.equal(await page.locator(card+' [data-gallery-role="fair"] .adaptive-gallery').count(),0,'removed fair left an empty gallery');
    ...
    assert.equal(await page.locator(card+' [data-gallery-role="fair"] .media-tile:not([data-kind="campaign"])').count(),0,'changed evidence retained supplementary assets');
    ...
    await page.waitForFunction(({s,n})=>document.querySelectorAll(s+' [data-gallery-role="fair"] .media-tile').length===n,{s:card,n:originalImages});await inspect();
