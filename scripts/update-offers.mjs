...
import {parseHtml,one,all,text,links,httpUrl} from './html-document.mjs';
...
const brands=(await read('brands.json')).brands;
...
const usefulLink=/食べ放題|コース|メニュー|料金|宴会|飲み放題|ビュッフェ|バイキング/;
...
function conditionsOf(s){
  const out=[];for(const [re,label] of [[/平日/,'平日'],[/土日祝|土・日・祝/,'土日祝'],[/ランチ/,'ランチ'],[/ディナー/,'ディナー'],[/予約限定|ネット予約|WEB予約|Web予約/,'予約条件あり'],[/店舗により|店舗によって|一部店舗/,'店舗条件あり']])if(re.test(s))out.push(label);return out;
}
function titleOf(s,kind){
  let v=clean(s).replace(/[￥¥]?\d[\d,]*\s*円\s*[（(]\s*税込[^）)]*[）)]/gu,' ').replace(/[（(]\s*税込[^）)]*[）)]/gu,' ').replace(/税込\s*[￥¥]?\s*\d[\d,]*\s*円?/gu,' ');
  v=clean(v);if(v.length>110)v=v.slice(0,107)+'…';
  if(!v)return kind==='drink'?'飲み放題':'食べ放題・コース';return v;
}
function extract(html,sourceUrl,brandId){
  const doc=parseHtml(html),rootNode=one(doc,'main,.area-contents,.contents,body')||doc;
  const rows=[];
  for(const node of all(rootNode,'p,li,tr,dd')){
    const s=clean(text(node));if(s.length<4||s.length>260||child.test(s)||addon.test(s))continue;
    const kind=drink.test(s)?'drink':course.test(s)?'course':null;if(!kind)continue;
    const p=taxPrice(s);if(kind==='course'&&!p.priceText&&!/食べ放題|ビュッフェ|バイキング/.test(s))continue;
    rows.push({brandId,kind,title:titleOf(s,kind),...p,conditions:conditionsOf(s),officialUrl:sourceUrl,sourceUrl,checkedAt:stamp,comparisonKey:sourceUrl+'|'+conditionsOf(s).join(','),rawText:s});
  }
  // Some official pages expose the useful price only in link text.
  for(const link of links(rootNode,sourceUrl)){
    const s=clean(link.title);if(s.length<4||s.length>220||child.test(s)||addon.test(s))continue;
    const kind=drink.test(s)?'drink':course.test(s)?'course':null;if(!kind)continue;
    const p=taxPrice(s);rows.push({brandId,kind,title:titleOf(s,kind),...p,conditions:conditionsOf(s),officialUrl:publicUrl(link.url)||sourceUrl,sourceUrl,checkedAt:stamp,comparisonKey:sourceUrl+'|'+conditionsOf(s).join(','),rawText:s});
  }
  const seen=new Set();return rows.filter(x=>{const k=[x.kind,x.title,x.priceText||'',x.officialUrl].join('|');if(seen.has(k))return false;seen.add(k);return true;}).slice(0,16);
}
async function collect(brand){
  ...
  let homeOk=false;
  try{
    const {html,finalUrl}=await request(brand.homeUrl);homeOk=true;add(finalUrl);
    const doc=parseHtml(html),rootNode=one(doc,'main,.area-contents,.contents,body')||doc;
  ...
  const found=[];let pagesRead=0;
  for(const url of urls.slice(0,7)){
    try{const {html,finalUrl}=await request(url);pagesRead++;found.push(...extract(html,finalUrl,brand.id));}
    catch(e){errors.push({brandId:brand.id,url,error:e.message});}
  }
  const seen=new Set(),offers=found.filter(x=>{const k=[x.kind,x.title,x.priceText||'',x.officialUrl].join('|');if(seen.has(k))return false;seen.add(k);return true;}).map((x,i)=>({...x,rank:i,id:crypto.createHash('sha256').update([brand.id,x.kind,x.title,x.priceText||'',x.officialUrl].join('|')).digest('hex').slice(0,16)}));
  if(!offers.length&&(!homeOk||errors.some(e=>e.brandId===brand.id))){
    const old=(previous.offers||[]).filter(x=>x.brandId===brand.id&&Date.now()-Date.parse(x.checkedAt)<MAX_AGE);
    return {offers:old,health:{brandId:brand.id,status:old.length?'last_known_good':'partial',pagesRead,offers:old.length}};
  }
...
const result={schemaVersion:1,updatedAt:stamp,offers:results.flatMap(x=>x.offers),sourceHealth:results.map(x=>x.health),errors};
