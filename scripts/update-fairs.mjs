...
import { linksFromHtml,relevantTitle,allowedPath,campaignId,textFromHtml,dateFields,imageFromHtml,deriveFields } from './fair-utils.mjs';
...
const previous=new Map((current.campaigns||[]).map(c=>[`${c.brandId}|${c.officialUrl}`,c]));
...
for(const brand of SOURCES.slice(0,4)){
  for(const source of brand.sources){
    try{
      ...
      const links=[...new Map(linksFromHtml(indexHtml,source.url).filter(x=>relevantTitle(x.title)&&allowedPath(brand.brandId,x.url)).map(x=>[x.url,x])).values()].slice(0,30);
      for(const link of links){
        try{
          const html=await fetchText(link.url);const text=textFromHtml(html);const d=dateFields(text);const x=deriveFields(link.title,text);const old=previous.get(`${brand.brandId}|${link.url}`);
          const c={id:campaignId(brand.brandId,link.url),brandId:brand.brandId,brandName:brand.name,title:link.title,officialUrl:link.url,sourceUrl:source.url,sourceType:source.type,campaignType:x.campaignType,priority:x.priority,startDate:d.startDate,startDateText:null,endDate:d.endDate,endDateText:d.endDateText,price:x.price,priceText:x.priceText,allYouCanEat:x.allYouCanEat,targetCourses:[],targetStores:[],targetAreas:[],regionScope:x.regionScope,limitedIngredients:x.limitedIngredients,weekdayCondition:x.weekdayCondition,conditions:[],imageUrl:imageFromHtml(html,link.url),firstSeenAt:old?.firstSeenAt||now,fetchedAt:now,lastVerifiedAt:now,confidence:(d.startDate||d.endDate||/期間限定|フェア|キャンペーン|コラボ|価格改定/.test(text))?0.9:0.72,contentHash:crypto.createHash('sha256').update(text.slice(0,20000)).digest('hex')};
