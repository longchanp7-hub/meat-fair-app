...
export function reviewSemanticallySupported(review,page,dates){
  ...
  const explicitTerms=(review.semanticTerms||[]).map(norm).filter(x=>x.length>=2);
  const titleTokens=explicitTerms.length?explicitTerms:norm(f.title).split(/[「」『』【】（）()・,，。:：〜～-]+/).filter(x=>x.length>=3);
  if(explicitTerms.length){
    if(!titleTokens.every(x=>title.includes(x)||body.includes(x)))return false;
  }else if(titleTokens.length&&!titleTokens.some(x=>title.includes(x)||body.includes(x)))return false;
