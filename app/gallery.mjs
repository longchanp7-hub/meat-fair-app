import {planGallery} from './gallery-plan.mjs?v=20260915-catalog1';
...
export function renderGallery(rows,brand,data,tab){
  ...
  return renderMediaAssets(assets,brand.name,'fair');
}
export function renderMediaAssets(assets,brandName,role='catalog'){
  if(!assets.length)return '';
  return `<div class="media-section" data-gallery-role="${html(role)}"><div class="gallery adaptive-gallery" data-media-count="${assets.length}" aria-label="${html(brandName)}の公式画像">${assets.map((a,i)=>{
    const w=Number(a.width)||0,h=Number(a.height)||0,ratio=w>0&&h>0?w/h:1;
    const label=a.label||(a.kind==='campaign'?(a.campaignType==='discount'?'割引・キャンペーン':'フェア'):a.kind==='detail'?'フェア内メニュー':'公式メニュー');
    const caption=mediaCaption(a.title);
    return `<a class="gallery-item media-tile ${i===0?'main':''}" href="${html(a.officialUrl)}" target="_blank" rel="noopener noreferrer" data-ratio="${ratio}" data-kind="${html(a.kind)}" data-group="${html(a.group||'')}" data-visual-weight="${Number(a.visualWeight)||1}" title="${html(a.title)}" aria-label="${html(a.title)}：公式ページを開く"><img src="${html(a.imageUrl)}" alt="${html(a.title)}" ${w&&h?`width="${w}" height="${h}"`:''} loading="lazy" decoding="async" referrerpolicy="no-referrer"><span class="media-caption${caption.price?' has-price':''}"><small>${html(label)}</small><span class="media-title">${html(caption.title)}</span>${caption.price?`<strong class="media-price">${html(caption.price)}</strong>`:''}</span></a>`;
  }).join('')}</div><div class="media-fallbacks" aria-live="polite"></div>${assets.some(a=>a.kind==='menu')?'<p class="media-disclaimer">店舗・曜日・料金などの条件は、各画像の公式ページで確認してください。</p>':''}</div>`;
}
let cleanups=[];
