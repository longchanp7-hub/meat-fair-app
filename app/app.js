...
let tab='active',brandFilter=null,brandsData,fairsData,storesData,mediaData,catalogData,offersData={offers:[]};
...
function brandCard(brand,rows){
 const sorted=sortCampaigns(rows),fairRows=selectFairRows(sorted),promotions=selectPromotions(sorted),h=health(brand.id);
 ...
 return`<article class="restaurant-card" id="brand-${esc(brand.id)}" data-brand-card="${esc(brand.id)}"><div class="restaurant-head"><div><div class="category">${esc(CAT_LABEL[brand.category]||'レストラン')}</div><h3>${esc(brand.name)}</h3><div class="restaurant-summary">${esc(TAB_LABEL[tab])}${h.status==='partial'?' ・ 一部要確認':''}</div></div><span class="brand-rank">${String(brand.order).padStart(2,'0')}</span></div>${renderGallery(sorted,brand,mediaData,tab)}<div class="restaurant-body">${body}${preview}${tab==='active'?renderCatalog(brand,catalogData,fairsData,mediaData,new Date(),offersData):''}${promotions.length?`<section class="catalog-section promotion-section"><h4>お得なキャンペーン・特典</h4>${promotions.map(campaignRow).join('')}</section>`:''}${brandAvailability(brand.id,sorted)}<a class="brand-official" href="${esc(safeUrl(brand.homeUrl))}" target="_blank" rel="noopener noreferrer">${esc(brand.name)} 公式サイトへ <span>↗</span></a></div></article>`;
}
function renderList(){
...
try{
 [brandsData,fairsData,storesData,mediaData,catalogData,offersData]=await Promise.all([getJson('./data/brands.json'),getJson('./data/fairs.json'),getJson('./data/stores.json').catch(()=>({stores:[]})),getJson('./data/gallery.json').catch(()=>({assets:[]})),getJson('./data/catalog.json').catch(()=>null),getJson('./data/offers.json').catch(()=>({offers:[]}))]);
...
function startAutomaticRefresh(){
 ...
 const update=async()=>{
  ...
  try{
   const [nextFairs,nextMedia,nextCatalog,nextOffers]=await Promise.all([getJson('./data/fairs.json'),getJson('./data/gallery.json').catch(()=>mediaData),getJson('./data/catalog.json').catch(()=>catalogData),getJson('./data/offers.json').catch(()=>offersData)]);
   ...
   if(Array.isArray(nextOffers?.offers))offersData=nextOffers;
   renderHeader();
