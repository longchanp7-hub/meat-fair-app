...
The latest main (b1f714d) introduced offers collection, reviewed fallback prices and new semantic layout tests while this work was in preview. The integration retains all those source files, data, tests, generic landscape stacking and semantic groups. The offers update/verification remains in the daily pipeline. The app uses one catalog renderer instead of duplicate fee sections. Explicitly reviewed fallback records are displayed with their original verification time for at most 48 hours; automatic loose offer snippets are not imported. The latest main's separate promotion grouping is retained, with complete booking conditions.
